...
class SimpleSampler(Sampler):
    def __init__(self, leg_mapping=None, high_lv_control=False, **kwargs):
        super(SimpleSampler, self).__init__(**kwargs)

        #LATER DELETE high_lv_control
        ...
        self.leg_mapping = leg_mapping
        self.high_lv_control = high_lv_control
...
def rollout(env, policy, path_length, ac_dim, render=False, speedup=1.0):
    Da = ac_dim or np.int64(env._wrapped_env.env.input_dim)
...
def rollouts(env, policy, path_length, n_paths, ac_dim=np.int64(3)):
    paths = list()
    for i in range(n_paths):
        paths.append(rollout(env, policy, path_length, ac_dim))
